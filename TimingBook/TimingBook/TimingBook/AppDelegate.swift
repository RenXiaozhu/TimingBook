...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if startWindow == nil
        {
            startWindow = UIWindow(frame: UIScreen.main.bounds)
            startWindow?.backgroundColor = UIColor.black
            startWindow?.isUserInteractionEnabled = false
            startWindow?.windowLevel = UIWindowLevelStatusBar+1.0
            startWindow?.isHidden = false
        }
